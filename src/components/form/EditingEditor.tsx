...
import type { FootageRange } from "@/lib/footage-drag";
import type { EditingWorkspaceControls } from "./EditingWorkspace";
...
export function EditingEditor({ filenameOverride, workspace }: { filenameOverride?: string; workspace?: EditingWorkspaceControls }) {
  ...
  type EditorView = "variations" | "video" | "shots" | "upload" | "clips" | "storyboards" | "render" | "captions" | "virality";
  const [localPanelTab, setPanelTabState] = useState<EditorView>("video");
  const panelTab = workspace?.view ?? localPanelTab;
  const hasEdit = !workspace || !!workspace.activeEdit;
  const setPanelTab = useCallback((tab: EditorView) => {
    if (workspace) { workspace.setView(tab === "shots" ? "storyboards" : tab); return; }
  ...
  const editClips = async (operation: TimelineOperation | { type: "undo" | "redo" }) => {
    ...
    try {
      await flushEditSaves(videoId);
      for (const [index, entry] of Object.entries(textDrafts)) await patchTextOverlays({ ...entry, shot_index: Number(index) });
      await textSaveQueue.current;
      if (textSaveError.current) throw new Error(textSaveError.current);
      for (const [index, note] of Object.entries(instructionDrafts)) if (!await saveNote(Number(index), note)) throw new Error("Could not save shot instructions");
      ...
      const index = operation.type === "move" ? operation.to : operation.type === "insert" || operation.type === "replace" || operation.type === "broll" ? operation.index : Math.min(selectedShot, saved.analysis.shots.length - 1);
  ...
  const addTimelineFootage = async (range: FootageRange, action: "insert" | "replace" | "broll", index = action === "insert" ? selectedShot + 1 : selectedShot, offset = 0) => {
    if (!workspace?.activeEdit || !videoId || retiming || rendering || brollBusy || noteSaving || textSaving) return false;
    try {
      const response = await fetch(`/api/master/${workspace.sourceId}/footage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ filename: range.filename }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not include footage");
      workspace.onFootageIncluded();
      return await editClips({ type: action, index, ...range, ...(action === "broll" ? { offset } : {}) });
    } catch (e) { setTimelineError(e instanceof Error ? e.message : "Could not add footage"); return false; }
  };
  ...
  const shot = hasEdit ? analysis?.shots[selectedShot] : undefined;
  ...
  const flushWorkspace = useRef<() => Promise<void>>(async () => {});
  flushWorkspace.current = async () => {
    if (!hasEdit) return;
    if (videoId) await flushEditSaves(videoId);
    if (timelineSaving.current || rendering || noteSaving || textAligning || brollBusy) throw new Error("Wait for the current edit to finish saving.");
    for (const [index, entry] of Object.entries(textDrafts)) await patchTextOverlays({ ...entry, shot_index: Number(index) });
    await textSaveQueue.current;
    if (textSaveError.current) throw new Error(textSaveError.current);
    for (const [index, note] of Object.entries(instructionDrafts)) if (!await saveNote(Number(index), note)) throw new Error("Could not save shot instructions.");
    if (noteShot !== null && noteDraft !== (editNotes[String(noteShot)] ?? "")) if (!await saveNote(noteShot, noteDraft)) throw new Error("Could not save shot instructions.");
    if (framing.document) await framing.flush();
    videoRef.current?.pause();
  };
  const registerFlush = workspace?.registerFlush;
  useEffect(() => registerFlush?.(() => flushWorkspace.current()), [registerFlush]);
          ...
          <section ref={editingPanelRef} aria-label="Editing panel" tabIndex={0} className={`flex flex-col gap-3 min-w-0 ${analysis || workspace ? styles.panel : ""}`}>
            ...
            {analysis && hasEdit && (
              <div className="flex shrink-0 flex-col gap-3 min-w-0">
                ...
                {!workspace && panelTab === "virality" && videoId && <ViralityReviewPanel videoId={videoId} storyboardId={searchParams.get("storyboard") ?? undefined} onSeek={project?.kind === "master" ? seekTo : undefined} />}
