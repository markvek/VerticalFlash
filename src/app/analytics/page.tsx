...
type RemakeMatch = { filename: string; displayName: string | null };
...
export default function AnalyticsPage() {
  ...
  // Score the winners (> MATCH_MIN_VIEWS) against local render manifests to
  ...
  const loadMatches = useCallback(async (rows: TikTokVideoStats[]) => {
    const winners = rows
      .filter((v) => v.viewCount > MATCH_MIN_VIEWS)
      .map((v) => ({
        id: v.id,
        title: v.title,
        duration: v.duration,
    ...
    try {
      ...
      // matching is decorative — never surface an error for it
    }
  }, []);
