...
export default function NewEditingProjectPage() {
  ...
  const categories = [...new Set((library?.videos ?? []).map(clip => clip.analysis?.category).filter((value): value is string => !!value))].sort();
  const dates = [...new Set((library?.videos ?? []).map(clip => (clip.date ?? clip.createdAt).slice(0, 10)))].sort().reverse();
  const filteredClips = (library?.videos ?? []).filter(clip =>
    (!category || clip.analysis?.category === category) && (!date || (clip.date ?? clip.createdAt).startsWith(date)) &&
    `${clip.filename} ${clipDescription(clip)} ${clipTags(clip).join(" ")}`.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      setStatus("Opening editing workspace...");
      ...
      router.push(`/editing/${encodeURIComponent(filename)}?view=storyboards`);
    } catch (e) {
      ...
      <div className="w-full max-w-3xl space-y-8 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold">
            Start an editing project
          </h1>
          <p className="mt-2 text-muted-foreground">
            Choose footage from your computer or library. {brand.name} proposes
            storyboards you can take straight into editing
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="space-y-3">
            ...
            {library && <div className="mb-3 flex flex-wrap gap-2">
              <input aria-label="Search footage" placeholder="Search footage or tags" className="min-w-40 flex-1 rounded-md border border-border bg-background px-3 py-2 text-sm" value={search} onChange={e => setSearch(e.target.value)} />
              <select aria-label="Footage category" className="rounded-md border border-border bg-background px-2 py-2 text-sm" value={category} onChange={e => setCategory(e.target.value)}><option value="">All categories</option>{categories.map(value => <option key={value}>{value}</option>)}</select>
              <select aria-label="Footage date" className="rounded-md border border-border bg-background px-2 py-2 text-sm" value={date} onChange={e => setDate(e.target.value)}><option value="">All dates</option>{dates.map(value => <option key={value}>{value}</option>)}</select>
            </div>}
            {library && !filteredClips.length && library.videos.length > 0 && <p className="text-sm text-muted-foreground">No footage matches these filters.</p>}
                ...
                {filteredClips.map((clip) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isSelected}
                      title={isSelected ? "Already selected" : "Select footage"}
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block truncate text-[10px] text-muted-foreground">{[clip.analysis?.category, ...clipTags(clip)].filter(Boolean).join(" · ")}</span>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              {submitting ? "Opening..." : "Continue to Storyboards"}
            </button>
            <button
              type="button"
              onClick={() => router.push("/")}
