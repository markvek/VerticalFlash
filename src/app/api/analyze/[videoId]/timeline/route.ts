  ...
  const task = previous.catch(() => {}).then(async () => {
    try {
      ...
      if (op.type === "undo" || op.type === "redo") {
        ...
        const { shots, order } = editTimeline(before, op, sources.map(s => s!.bounds));
        if (op.type === "trim") {
        ...
        if (project?.kind === "cutdown") {
          ...
          metadata.beats = order.map((old, i) => ({ ...project.beats[old], source_start: shots[i].source_start, source_end: shots[i].source_end, start: shots[i].start_time, end: shots[i].end_time, text: shots[i].spoken_text }));
          next.metadata = json(metadata);
        }
        for (const kind of kinds) {
          if (!snapshot[kind]) continue;
          const doc = JSON.parse(snapshot[kind]!);
          if (kind === "recommendations") doc.shots = remapIndexed(doc.shots, order);
